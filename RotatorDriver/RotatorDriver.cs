    ...
    public class Rotator : ReferenceCountedObjectBase, IRotatorV3
    {
        ...
        public Rotator()
        {
            try
            {
                ...
                tl = new TraceLogger("", "ASCOM.byHrast.Rotator");
                ReadProfile(); // Read device configuration from the ASCOM Profile store, including the trace state

                tl.LogMessage("Rotator", "Starting initialisation");
                tl.LogMessage("Rotator", $"ProgID: {driverID}, Description: {driverDescription}");
            ...
            {
                tl.LogMessageCrLf("Rotator", $"Initialisation exception: {ex}");
